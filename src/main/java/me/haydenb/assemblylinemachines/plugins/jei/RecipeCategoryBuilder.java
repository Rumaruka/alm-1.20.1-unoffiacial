...
public class RecipeCategoryBuilder {
	...
	private TriConsumer<Recipe<?>, GuiGraphics, Pair<Double, Double>> draw = null;
	...
	RecipeCategoryBuilder draw(TriConsumer<Recipe<?>, GuiGraphics, Pair<Double, Double>> triConsumer) {
	...
	<R extends Recipe<?> & IRecipeCategoryBuilder> IRecipeCategory<R> build(Class<R> clazz){
		...
		class ALMRecipeCategory implements IRecipeCategory<R>, IInfoProvider{
            ...
            @Override
			public void draw(R recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics stack, double mouseX, double mouseY) {
				if(progressBar != null) {

		...
		if(CATEGORY_REGISTRY.isEmpty()) {
			...
			CATEGORY_REGISTRY.add(new RecipeCategoryBuilder(guiHelper, "grinding", "Grinding Crafting")
					...
					.tooltip((pP1, pP2, tooltip) -> {
						if(pP1 instanceof GrinderCrafting recipe && pP2 instanceof ItemStack stack) {
							if(recipe.chanceToDouble != 0 && stack.getCount() == recipe.getResultItem(Minecraft.getInstance().level.registryAccess()).getCount() * 2) tooltip.add(Component.literal(String.format("%.0f%%", (recipe.chanceToDouble * 100f)) + " Chance").withStyle(ChatFormatting.AQUA));
						}
			...
			CATEGORY_REGISTRY.add(new RecipeCategoryBuilder(guiHelper, "pneumatic", "Pneumatic Compressor")
					...
					.draw(new TriConsumer<>() {
						...
						@Override
						public void accept(Recipe<?> k, GuiGraphics v, Pair<Double, Double> s) {
			...
			CATEGORY_REGISTRY.add(new RecipeCategoryBuilder(guiHelper, "entropy_reactor_output", "Entropy Reactor Waste")
					...
					.tooltip((pP1, pP2, tooltip) -> {
						if(pP1 instanceof EntropyReactorCrafting recipe && pP2 instanceof ItemStack stack) {
							if(recipe.getResultItem(Minecraft.getInstance().level.registryAccess()).is(stack.getItem())) {
								...
								for(int i = 1; i - 1 < BlockMachines.ELECTRIC_DRAW_DATA.length; i++) {
									if(timer.getValue() > i) {
										for(int[] scanbar : BlockMachines.ELECTRIC_DRAW_DATA[i - 1]) {
											v.fill( 41 + i, scanbar[0] - 29, 42 + i, scanbar[1] - 29, colors[scanbar[2]]);
										}
									}
								}
							}

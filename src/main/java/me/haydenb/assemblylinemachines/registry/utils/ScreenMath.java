...
public class ScreenMath {
	...
	@OnlyIn(Dist.CLIENT)
	public static void renderItemSlotBoundScaledText(GuiGraphics gg,Font fr, int xpos, int ypos, float scale, String text) {

		xpos = Math.round(xpos * (1.0f / scale));
		ypos = Math.round(ypos * (1.0f / scale));
		PoseStack mx =gg.pose();
		...
		gg.drawString(fr,text, xpos + 19 - 2 - fr.width(text), ypos + 6 + 3, 16777215
		);
	...
	@OnlyIn(Dist.CLIENT)
	public static void renderScaledText(GuiGraphics gg,Font fr, int xpos, int ypos, float scale, String text, boolean dropShadow, int color) {
		...
		mx.scale(scale, scale, scale);
		BufferSource irendertypebuffer$impl = MultiBufferSource.immediate(Tesselator.getInstance().getBuilder());
		gg.drawString(fr,text, xpos, ypos, color
		);
		irendertypebuffer$impl.endBatch();
	}

	@OnlyIn(Dist.CLIENT)
	public static void renderScaledText(GuiGraphics gg,Font fr, int xpos, int ypos, float scale, String text) {

		renderScaledText(gg,fr, xpos, ypos, scale, text, false, 0xffffff);
	}
	...
	@OnlyIn(Dist.CLIENT)
	public static void drawCenteredStringWithoutShadow(GuiGraphics gg, Font pFont, Component pText, int pX, int pY, int pColor) {
		FormattedCharSequence formattedcharsequence = pText.getVisualOrderText();
		gg.drawString(pFont, formattedcharsequence, pX - pFont.width(formattedcharsequence) / 2, pY, pColor,false);
	}

	@OnlyIn(Dist.CLIENT)
	public static void drawCenteredStringWithoutShadow(GuiGraphics gg, Component pText, int pX, int pY) {
		Minecraft mc = Minecraft.getInstance();
		drawCenteredStringWithoutShadow(gg, mc.font, pText, pX, pY, 4210752);
	}

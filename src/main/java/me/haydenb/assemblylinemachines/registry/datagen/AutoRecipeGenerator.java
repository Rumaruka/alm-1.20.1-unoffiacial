...
public class AutoRecipeGenerator extends RecipeProvider {
			...
			for(File file : fileList) {
				if(file.isFile() && FilenameUtils.getExtension(file.getName()).equals("json")) {
					...
					try {
						...
						consumer.accept(new AdvancementlessResult(new ResourceLocation(AssemblyLineMachines.MODID, "smelting/" + rlPath), recipe.getIngredients().get(0), recipe.assemble(null, Minecraft.getInstance().level.registryAccess()).getItem(), recipe.getExperience(), recipe.getCookingTime() * 2));
	...
	public static class AdvancementlessResult implements FinishedRecipe{
		...
		public AdvancementlessResult(ResourceLocation rl, Item result, int numberOfResult, List<String> pattern, Map<Character, Ingredient> key) {
			this.recipe = new ShapedRecipeBuilder.Result(rl, result, numberOfResult, "", CraftingBookCategory.MISC, pattern, key, null, null,false);
		}

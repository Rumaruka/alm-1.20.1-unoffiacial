...
public enum BlockCorruptOres{
	...
	public static void createCorruptOres(){
		for(BlockCorruptOres ores : BlockCorruptOres.values()) {
			...
			Registry.createBlock("corrupt_" + ore + "_ore", new CorruptBlock(Block.Properties.of().sound(SoundType.STONE).strength(3f, 9f).requiresCorrectToolForDrops(), BlockTags.MINEABLE_WITH_PICKAXE, BlockTags.NEEDS_DIAMOND_TOOL), true);
			Registry.createBlock("corrupt_basalt_" + ore + "_ore", new CorruptBlockWithAxis(Block.Properties.of().sound(SoundType.STONE).strength(5f, 15f).requiresCorrectToolForDrops(), BlockTags.MINEABLE_WITH_PICKAXE, BlockTags.NEEDS_DIAMOND_TOOL, false, true), true);
		}
	}
}
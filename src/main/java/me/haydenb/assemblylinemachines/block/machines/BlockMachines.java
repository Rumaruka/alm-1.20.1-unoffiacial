				...
				for(int i = 1; i - 1 < KINETIC_DRAW_DATA.length; i++) {
					if(p > (0.0416f * i)) {
						for(int[] scanbar : KINETIC_DRAW_DATA[i - 1]) {
							gg.fill( x + scanbar[0], y + (43 - i), x + scanbar[1], y + (44 - i), colors[scanbar[2]]);
						}
					}
				}
			}
				...
				for(int i = 1; i - 1 < ELECTRIC_DRAW_DATA.length; i++) {
					if(p > (0.066f * i)) {
						for(int[] scanbar : ELECTRIC_DRAW_DATA[i - 1]) {
							gg.fill( x + (94 + i), y + scanbar[0], x + (95 + i), y + scanbar[1], colors[scanbar[2]]);
						}
					}
				}
			}
				...
				for(int i = 1; i - 1 < MKII_DRAW_DATA.length; i++) {
					if(p > (0.066f * i)) {
						for(int[] scanbar : MKII_DRAW_DATA[i - 1]) {
							gg.fill( x + scanbar[0], y + (41 + i), x + scanbar[1], y + (42 + i), colors[scanbar[2]]);
							gg.fill( x + scanbar[0] + 54, y + (41 + i), x + scanbar[1] + 54, y + (42 + i), colors[scanbar[2]]);
						}
					}
				}
			}

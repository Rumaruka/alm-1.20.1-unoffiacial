...
public enum ItemTiers {
    ...
    TITANIUM(ToolDefaults.TITANIUM, "titanium", List.of(Tiers.DIAMOND), List.of(Tiers.NETHERITE), false, () -> {
        return Ingredient.of(Registry.getItem("titanium_ingot"));
    }),
    STEEL(ToolDefaults.STEEL, "steel", List.of(Tiers.IRON), List.of(Tiers.DIAMOND), false, () -> {
        return Ingredient.of(Registry.getItem("steel_ingot"));
    }),
    CRANK(ToolDefaults.CRANK, null, List.of(Tiers.DIAMOND), List.of(Tiers.NETHERITE), false, () -> {
        return Ingredient.of(TagKey.create(Keys.ITEMS, new ResourceLocation("assemblylinemachines", "precious_gears")));
    }),
    MYSTIUM(ToolDefaults.MYSTIUM, "mystium", List.of(Tiers.NETHERITE), List.of(), true, () -> {
        return Ingredient.of(Registry.getItem("mystium_ingot"));
    }),
    NOVASTEEL(ToolDefaults.NOVASTEEL, null, List.of(ItemTiers.MYSTIUM.getItemTier()), List.of(), true, () -> {
        return Ingredient.of(Registry.getItem("novasteel_ingot"));
    }),
    ...
    ItemTiers(ToolDefaults toolDefaults, String armorSetName, List<Object> tiersAfter, List<Object> tiersBefore, boolean hasTag, Supplier<Ingredient> ingredient) {
        ...
        class ItemTier implements Tier {
            ...
            public ItemTier() {
                ...
                if (tiersAfter != null && tiersBefore != null)
                    TierSortingRegistry.registerTier(this, new ResourceLocation(AssemblyLineMachines.MODID, ItemTiers.this.toString().toLowerCase()), tiersAfter, tiersBefore);
            }
        ...
        class ArmorTier implements ArmorMaterial {
            ...
            public int getDurabilityForType(ArmorItem.Type pType) {
                return Math.round(getStat(Stats.DURABILITY).intValue() / switch (pType) {
                    case HELMET -> 3f;
                    case CHESTPLATE -> 2f;
                    case LEGGINGS -> 2.5f;
                    case BOOTS -> 3.25f;
                    default -> throw new UnsupportedOperationException("Unsupported case value: " + pType.toString());
                });
            }

            @Override
            public int getDefenseForType(ArmorItem.Type pType) {
                return Math.round(getStat(Stats.D_REDUCTION).intValue() * switch (pType) {
                    case HELMET -> 1f;
    ...
    public static enum ToolDefaults {
        ...
        ToolDefaults(List<Stats> statTypes, Number... data) {
            if (statTypes.size() != data.length)
                throw new IllegalArgumentException("Stat types does not equal data length.");

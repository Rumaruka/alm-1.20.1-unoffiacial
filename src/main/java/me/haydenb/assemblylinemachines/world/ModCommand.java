...
public class ModCommand {
	...
	private static int makeCreative(CommandContext<CommandSourceStack> context) throws CommandSyntaxException{
		...
		Optional<MutableComponent> result = ItemCreativeUpgradeKit.makeCreative(source.getEntityOrException().level(), pos);
		if(result.isPresent()) {
	...
	private static int fluidInChunk(CommandContext<CommandSourceStack> context) throws CommandSyntaxException{
		...
		LazyOptional<IChunkFluidCapability> lazy = CapabilityChunkFluids.getChunkFluidCapability(entity.level().getChunkAt(pos));
		if(lazy.isPresent()) {
